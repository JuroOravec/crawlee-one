 ...
 *
 * The GitHub Actions workflow (.github/workflows/benchmark.yml) reads
...
export const benchConfig = {
  ...
  /** Vitest bench JSON output (generated by `npm run bench:gen`). */
  vitestOutputFile: fromRoot('benchmarks', 'temp', 'results.json'),

  /** Sidecar file storing test results metadata (written by helpers.ts, consumed by run-bench.ts generate). */
  sidecarFile: fromRoot('benchmarks', 'temp', 'sidecar.json'),

  /** Dashboard JSON for github-action-benchmark (written by run-bench.ts generate). */
  dashboardOutputFile: fromRoot('benchmarks', 'temp', 'dashboard.json'),

  /** Root directory for persisted benchmark results (committed to the repo). */
  resultsDir: fromRoot('benchmarks', 'data'),
