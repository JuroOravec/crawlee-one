...
├── lib/actor/actor.ts     # Top-level crawleeOne()
...
1. **`crawleeOne()`** (in `src/lib/actor/actor.ts`) is the main entry point. It resolves input, creates scoped push/metamorph functions, registers routes, instantiates the correct crawler class, and runs it.
...
The build has two steps (see `pnpm build`):
...
```sh
pnpm bench                 # interactive run with table output
pnpm bench:gen             # run benchmarks and transform into rich archive + dashboard JSON
```
...
ESLint (flat config) + Prettier. Run `pnpm lint:fix` before committing.
...
CI runs this in the `checks` job after `pnpm install` and before lint/build/test.
...
**`branch-protection.ts`** -- Validates that GitHub branch protection required status checks match the CI workflow job names. Parses `.github/workflows/tests.yml` to derive expected job names (expanding matrix combinations), fetches branch protection via `gh api`, and compares. On mismatch, prints the exact `gh api` command to fix it. Requires `gh` CLI with admin access; skips gracefully if unavailable.
...
Tags follow the format `crawlee-one@{version}` (e.g. `crawlee-one@1.2.3`). The release workflow only triggers for tags matching this pattern.
