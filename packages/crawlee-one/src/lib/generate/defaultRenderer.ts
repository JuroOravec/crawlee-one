import type { ReadmeRenderer } from '../config/types.js';
 ...
 * template engine.
 */
export const defaultReadmeRenderer: ReadmeRenderer<Partial<DefaultReadmeInput>> = (args) => {
  const { actorSpec, input } = args;
  const spec = actorSpec as Record<string, any> | undefined;
  const t = input ?? ({} as Partial<DefaultReadmeInput>);
