...
describe('addRequests', () => {
  it('pushes requests to the queue', async () => {
    ...
    await addRequests(requests as any, { io });

    expect(reqQueue.addRequests).toHaveBeenCalledTimes(1);
    expect(reqQueue.addRequests).toHaveBeenCalledWith(requests, {});
  });
  ...
  it('converts single item to array', async () => {
    ...
    await addRequests(request as any, { io });

    expect(reqQueue.addRequests).toHaveBeenCalledWith([request], {});
  });
  ...
  it('applies transform function', async () => {
    ...
    await addRequests(requests as any, {
      io,
  ...
  it('opens named request queue when requestQueueId is provided', async () => {
    ...
    await addRequests([{ url: 'https://example.com', uniqueKey: '1' }] as any, {
      io,
  ...
  it('passes RequestQueueOperationOptions through', async () => {
    const reqQueue = createMockRequestQueue();
    const io = createMockIO({ reqQueue });
...
describe('addRequests with maxCount', () => {
  it('limits requests when maxCount is set and queue has capacity', async () => {
    ...
    await addRequests(requests as any, { io, maxCount: 10 });
  ...
  it('allows all requests when queue is empty and maxCount is large enough', async () => {
    ...
    await addRequests(requests as any, { io, maxCount: 100 });
