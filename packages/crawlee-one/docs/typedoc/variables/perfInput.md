[**crawlee-one**](../README.md)

---

[crawlee-one](../README.md) / perfInput

# Variable: perfInput

> `const` **perfInput**: `object`

Defined in: [packages/crawlee-one/src/lib/input.ts:724](https://github.com/JuroOravec/crawlee-one/blob/main/packages/crawlee-one/src/lib/input.ts#L724)

Common input fields related to performance which are not part of the CrawlerConfig

## Type Declaration

### batchSize

> **batchSize**: `IntegerField`\<`number`, `string`, `ZodOptional`\<`ZodNumber`\>\>

### batchWaitSecs

> **batchWaitSecs**: `IntegerField`\<`number`, `string`, `ZodOptional`\<`ZodNumber`\>\>
