[**crawlee-one**](../README.md)

***

[crawlee-one](../README.md) / LlmComparisonReport

# Interface: LlmComparisonReport\<TData\>

Defined in: packages/crawlee-one/src/lib/llmCompare/types.ts:50

Report from runLlmModelComparison

## Type Parameters

### TData

`TData` = `unknown`

## Properties

### byId

> **byId**: `Record`\<`string`, \{\[`modelId`: `string`\]: `TData` \| `null`; \}\>

Defined in: packages/crawlee-one/src/lib/llmCompare/types.ts:55

Per ID: each model's result (null if model failed for that ID)

***

### models

> **models**: [`LlmModelReport`](LlmModelReport.md)[]

Defined in: packages/crawlee-one/src/lib/llmCompare/types.ts:53

***

### sourceOfTruthId

> **sourceOfTruthId**: `string`

Defined in: packages/crawlee-one/src/lib/llmCompare/types.ts:52

***

### totalIds

> **totalIds**: `number`

Defined in: packages/crawlee-one/src/lib/llmCompare/types.ts:51
