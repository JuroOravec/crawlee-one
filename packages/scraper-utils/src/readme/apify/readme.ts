...
export const renderApifyReadme: ReadmeRenderer<ApifyReadmeInput> = async (args) => {
  ...
  README_HOOK.forEach((key) => {
    ...
    eta.loadTemplate(`@hook.${key}`, template || '');
  });
  ...
  Object.entries(templates.features).forEach(([key, feat]) => {
    ...
    eta.loadTemplate(`@feat.${key}.title`, title);
    eta.loadTemplate(`@feat.${key}.mainText`, mainText);
    eta.loadTemplate(`@feat.${key}.afterBegin`, afterBegin ?? '');
    eta.loadTemplate(`@feat.${key}.beforeEnd`, beforeEnd ?? '');
  });
  // Define templates for 'include(...)'s for perf table hooks
  Object.entries(templates.perfTables || {}).forEach(([key, perfTable]) => {
    perfTable.rows.forEach((row) => eta.loadTemplate(`@perfTable.${key}.row.${row.rowId}`, row.template)); // prettier-ignore
    perfTable.cols.forEach((col) => eta.loadTemplate(`@perfTable.${key}.col.${col.colId}`, col.template)); // prettier-ignore
  });
